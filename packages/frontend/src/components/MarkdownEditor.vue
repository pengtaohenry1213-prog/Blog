<template>
  <div class="v_loading" v-loading="isLoading">
    <div ref="editorRef" />
  </div>
</template>
<script setup>
import { ref, onMounted, onBeforeUnmount, watch, defineProps, defineEmits, nextTick } from 'vue';
// import { ElLoading } from 'element-plus' // 如果用指令，确保已全局注册；否则用服务模式
...
let stopModelValueWatch = null;
let stopEditorReadyWatch = null;
...
function initMessageChannel() {
  ...
  try {
    ...
    markdownWorker.onmessage = event => {
      console.log('markdownWorker.onmessage -> event: ', event);
    };
    ...
    workerPort.onmessage = event => {
      ...
      if (type === 'analysis' || type === 'validation' || type === 'keywords') {
        isLoading.value = false; // 这里就是“handleWorkerDone”要做的事
      }
      ...
      if (vditor.value && props.modelValue) {
        try {
          ...
          nextTick(() => {
            ...
            // vditor.value.tip('已加载', 1500) // 可选：短暂提示
            // vditor.value.tip('', 0);
          });
        }
      }
...
stopModelValueWatch = watch(
  ...
  val => {
    ...
    try {
      isLoading.value = true;
      // vditor.value.tip(LOADING_TEXT, 0);
...
onBeforeUnmount(() => {
  ...
  if (stopModelValueWatch) {
    stopModelValueWatch();
    stopModelValueWatch = null;
  }
  if (stopEditorReadyWatch) {
    stopEditorReadyWatch();
    stopEditorReadyWatch = null;
  }
